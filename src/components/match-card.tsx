...
export function MatchCard({
  ...
  return (
    <div
      className={`card-shadow overflow-hidden rounded-2xl border bg-panel transition ${
        match.status === "live" && !bettingClosed ? "border-lay/40 ring-live" : "border-line"
      }`}
    >
      {match.image_url ? (
      ...
      <div className="flex items-center justify-between gap-2 border-b border-line bg-panel-2/70 px-4 py-2.5">
        ...
        <div className="flex items-center gap-2">
          {match.end_time && !bettingClosed ? (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-gold/30 bg-gold/10 px-2.5 py-1 text-[11px] font-black tabular-nums text-gold">
                ...
                {(["A", "B"] as const).map((side) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-between gap-2 rounded-xl border px-3 py-3 text-left transition disabled:cursor-not-allowed disabled:opacity-50 ${
                        isSel
                          ? "border-brand bg-brand/15 ring-2 ring-brand/25 glow-brand"
                          : isWinner || backedSide === side
                            ? "border-brand/40 bg-brand/5"
                            : "border-line bg-panel-2 hover:border-brand/40 hover:bg-panel"
                      }`}
                    >
                      <span className="flex min-w-0 items-center gap-2">
                        <span
                          className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-[11px] font-black ${
                            isSel ? "bg-brand text-white" : "bg-panel text-ink/70"
                          }`}
                        >
                      ...
                      <span className={`shrink-0 tabular-nums text-sm font-extrabold ${isSel ? "text-brand" : "text-back"}`}>
                        {rate.toFixed(2)}
                      </span>
                    </button>
                  );
        ...
        <form action={formAction} className="border-t border-line bg-panel-2/70 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-full rounded-lg border border-line bg-panel-2 px-3 py-2.5 text-sm text-ink outline-none focus:border-brand focus:ring-2 focus:ring-brand/20"
            />
            <SubmitButton idleClass="bg-play text-white glow-play px-5">Place</SubmitButton>
          </div>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {QUICK.map((q) => (
              <button
                ...
                className="rounded-md border border-line bg-panel-2 px-2.5 py-1 text-xs font-semibold text-ink/70 hover:border-brand/40 hover:text-ink"
              >
                +{coins(q)}
              </button>
            ))}
            <button
              type="button"
              onClick={() => setStake("")}
              className="rounded-md border border-line bg-panel-2 px-2.5 py-1 text-xs font-semibold text-muted hover:text-ink"
            >
              Clear
            </button>
          </div>
